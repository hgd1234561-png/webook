...
type Service struct {
	svc  sms.TencentService
	repo repository.AsyncSmsRepository
	l    logger.Logger
}

func NewService(svc sms.TencentService, repo repository.AsyncSmsRepository, l logger.Logger) *Service {
	res := &Service{
		svc:  svc,
		repo: repo,
		l:    l,
	...
	case nil:
		...
		if err != nil {
			...
			s.l.Error("执行异步发送短信失败",
				logger.Field{Key: "err", Val: err}, logger.Field{Key: "id", Val: as.Id})
		}
		res := err == nil
		...
		if err != nil {
			s.l.Error("执行异步发送短信成功，但是标记数据库失败",
				logger.Field{Key: "err", Val: err}, logger.Field{Key: "id", Val: as.Id}, logger.Field{Key: "res", Val: res})
		}
